...
type BaseApp struct { //nolint: maligned
	...
	mempool         map[sdk.TransactionType]mempool.Mempool // application side mempool
...
func NewBaseApp(
	...
	if app.mempool == nil {
		app.mempool = make(map[sdk.TransactionType]mempool.Mempool)
		//app.SetMempool(mempool.NoOpMempool{}, sdk.MATCH)
	}
	...
	if app.processProposal == nil {
		app.SetProcessProposal(app.SelfProcessProposal())
	}

	if app.prepareProposal == nil {
		app.SetPrepareProposal(app.SelfPrepareProposal())
	}
...
func (app *BaseApp) SetMatching(match func(price float32)) {
	app.matching = match
}
...
func (app *BaseApp) GetMemPoolTxs() (retBuy []sdk.Tx, retSell []sdk.Tx) {
	retBuy = app.mempool[sdk.BUY].GetTxsByPool()
	retSell = app.mempool[sdk.SELL].GetTxsByPool()
	return
}
func (app *BaseApp) SetMemPoolTxs(buy []sdk.Tx, sell []sdk.Tx) {
	app.mempool[sdk.BUY].SetTxsToPool(buy)
	app.mempool[sdk.SELL].SetTxsToPool(sell)
}
func (app *BaseApp) InsertTxToNoopPool(tx sdk.Tx) {
	app.mempool[sdk.MATCH].Insert(nil, tx)
}
...
func (app *BaseApp) runTx(mode runTxMode, txBytes []byte) (gInfo sdk.GasInfo, result *sdk.Result, anteEvents []abci.Event, priority int64, err error) {
	...
	if mode == runTxModeCheck {
		txType, isOk := msgs[0].(sdk.TxSellBuy)
		if isOk {
			if txType.GetTxType() != sdk.PRICE {
				err = app.mempool[txType.GetTxType()].Insert(ctx, tx)
				if err != nil {
					return gInfo, nil, anteEvents, priority, err
				}
			} else {
				app.price = msgs[0].(sdk.TxSellBuy).GetPrice()
			}
		} else {
			err = app.mempool[sdk.NORMAL].Insert(ctx, tx)
			if err != nil {
				return gInfo, nil, anteEvents, priority, err
			}
		}
		...
		_, isOk := msgs[0].(sdk.TxSellBuy)
		memType := sdk.NORMAL
		if isOk {
			memType = sdk.MATCH
		}
		err = app.mempool[memType].Remove(tx)
...
//			_, isNoOp := app.mempool[sdk.NORMAL].(*mempool.NoOpMempool)
...
//			)
//
//			iterator := app.mempool[sdk.NORMAL].Select(ctx, req.Txs)
//			for iterator != nil {
//				memTx := iterator.Tx()
//
...
//				if err != nil {
//					err := app.mempool[sdk.NORMAL].Remove(memTx)
//					if err != nil && !errors.Is(err, mempool.ErrTxNotFound) {
//						panic(err)
//					}
//
...
//		}
//	}
func (app *BaseApp) SelfPrepareProposal() sdk.PrepareProposalHandler {
	return func(ctx sdk.Context, req abci.RequestPrepareProposal) abci.ResponsePrepareProposal {
		// If the mempool is nil or a no-op mempool, we simply return the transactions
		// requested from Tendermint, which, by default, should be in FIFO order.
		//_, isNoOp := app.mempool[sdk.MATCH].(mempool.NoOpMempool)
		//if app.mempool == nil || isNoOp {
		//	return abci.ResponsePrepareProposal{Txs: req.Txs}
		//}
		if app.mempool[sdk.NORMAL].CountTx() > 0 {
			_, isNoOp := app.mempool[sdk.NORMAL].(*mempool.NoOpMempool)
			if app.mempool == nil || isNoOp {
				return abci.ResponsePrepareProposal{Txs: req.Txs}
			}
			...
			pool := app.mempool[sdk.MATCH].(*mempool.NoOpMempool)
			var (
				txsBytes  [][]byte
				byteCount int64
			)
			app.matching(app.price)

			poolTx := pool.GetTxsByPool()
...
//		}
//	}
func (app *BaseApp) SelfProcessProposal() sdk.ProcessProposalHandler {
	return func(ctx sdk.Context, req abci.RequestProcessProposal) abci.ResponseProcessProposal {
		for _, txBytes := range req.Txs {
			_, err := app.txDecoder(txBytes)
			if err != nil {
