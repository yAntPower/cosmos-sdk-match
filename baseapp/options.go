...
func SetMempool(mempool mempool.Mempool, poolType sdk.TransactionType) func(*BaseApp) {
	return func(app *BaseApp) { app.SetMempool(mempool, poolType) }
}
...
func (app *BaseApp) SetMempool(mempool mempool.Mempool, t sdk.TransactionType) {
	if app.sealed {
		panic("SetMempool() on sealed BaseApp")
	}
	app.mempool[t] = mempool
}
