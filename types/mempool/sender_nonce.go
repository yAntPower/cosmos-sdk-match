...
/*
senderNonceMempool 是一个内存池，用于对发送者内的交易进行优先级排序
按随机数，最低的优先，但在每次迭代中选择一个随机发件人。
内存池通过以下方式迭代：
1) 为每个发送者维护一个单独的 nonce 有序 tx 列表
2) 对于每次选择迭代，随机选择一个发件人并从他们的列表中选择下一个随机数排序的 tx
3) 重复1,2直到内存池耗尽
请注意 PrepareProposal 可以选择在到达之前停止迭代
如果达到 maxBytes 则结束。

senderNonceMempool是一个在发送器中优先处理事务的内存池
到目前为止，最低的第一个，但在每次迭代中选择一个随机的发送者。
内存池被迭代:
1)维护每个发送者的nonce ordered txs的单独列表
2)对于每个选择迭代，随机选择一个发送者，并从他们的列表中选择下一个nonce ordered tx
3)重复1、2，直到内存池耗尽
注意，PrepareProposal可以选择在到达
如果达到maxBytes则结束。
*/
...
func (senderNonceMempool) GetTxsByPool() []sdk.Tx {
	return nil
}
func (mp senderNonceMempool) SetTxsToPool(txs []sdk.Tx) {
	//mp.Txs = txs
}
