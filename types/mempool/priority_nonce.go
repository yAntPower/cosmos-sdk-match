...
priorityNonceMempool是一个存储txs的内存池实现
在一个由2个维度组成的部分有序集合中:优先级和发送时间
(序号)。在内部，它使用一个优先级有序跳跃表和一个
按sender-nonce(序列号)排序的每个sender的跳过列表。当
来自同一发送者的多个TXS，它们并不总是可比较的
优先于其他发送方TXS，必须由两个发送方立即部分排序
和优先级。
*/
...
func (priorityNonceMempool) GetTxsByPool() []sdk.Tx {
	return nil
}
func (mp priorityNonceMempool) SetTxsToPool(txs []sdk.Tx) {
	//mp.Txs = txs
}
